...
const PopUpAccount = ({ isOpen, onClose, Username, Email }) => {
    ...
    <div className="fixed inset-0 z-40" onClick={onClose} >
      ...
      >
        <div className="flex items-start gap-3">
          <div className="h-12 w-12 md:h-14 md:w-14 rounded-full bg-bistre shrink-0"></div>
          <div className="min-w-0">
            <h3 className="font-semibold text-sm md:text-sm leading-tight">{Username.name}</h3>
            {/* <p className="text-xs md:text-sm text-white/95">Grade 12</p> */}
            <p className="text-xs md:text-sm text-white/95 break-all">{Email.email}</p>
          </div>
        </div>
