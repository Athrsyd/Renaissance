...
const WelcomeDash = ({ user }) => {
  return (
    <>
      <div className="relative bg-[#F8F3E0] w-full mt-6 p-6 lg:py-6 lg:px-10 border-[1.5px] border-[#9B7A5B]/30 rounded-2xl overflow-hidden">
        <div className="flex flex-col relative z-10 w-full lg:w-[65%] gap-2">
          <h1 className="text-2xl font-monstserrat font-semibold text-[#39221C]">
            <span className="text-icon">Hello,</span>{" "}
            {user.name || "Renaissance Learner"}!
          </h1>
        ...
        {/* <img
          src={Buku}
          className="hidden lg:block absolute z-0 right-0 -bottom-2 top-2 w-64 mr-3 opacity-90"
        /> */}
      </div>
    </>
  );
