  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center text-center">
        ...
        <h1 className="text-xl sm:text-2xl font-semibold font-monstserrat">
          {currentNormal} <span className="text-icon">{currentHighlight}</span>
          <span className="animate-pulse">|</span>
        </h1>
        <p className="text-black text-[11px] sm:text-sm font-semibold mt-2 max-w-xl font-monstserrat">
      ...
      <div className="w-full max-w-2xl bg-bistre p-2 md:p-4 rounded-2xl space-y-2">
        {quickQuestions.map((q, i) => (
          <button
            ...
            className="w-full bg-icon/20 backdrop-blur-md shadow-lg text-white py-2 rounded-full text-[10px] sm:text-sm font-semibold hover:bg-[#6b4a3c]"
          >
            {q}
          </button>
        ))}
