...
const TimelineBab = ({
  ...
  const matematikaProgress = modulProgress.filter(
    (item) =>
      mathModulesIds.includes(Number(item?.modul_id)) ||
      (item?.mapel || "").toLowerCase() === "matematika"
  );

  const getModuleProgress = (moduleId) => {
    return matematikaProgress.find(
        ...
        {isProgressLoading && (
          <div className="relative w-4/5 flex flex-col mt-5 gap-4">
            {[1, 2, 3].map((item) => (
              <Skeleton
                key={item}
                height={52} style={{ borderRadius: "0.75rem" }} />
            ))}
          </div>
        )}
                    ...
                    <>
                      ...
                      <div className="flex flex-row items-center">
                        <div className="flex flex-col items-center mr-5">
                          {/* Circle */}
                          <div
                            className={`w-10 h-10 rounded-full ${index === modul.length - 1 ? "-mt-13" : ""
