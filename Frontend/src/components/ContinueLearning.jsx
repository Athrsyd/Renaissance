...
const ContinueLearningCard = ({ item, index }) => {
    return (
        <div className={`px-10 py-5 ${index % 2 === 0 ? 'bg-bistre' : 'bg-beige'} ${item.progress ===100 ?'hidden':'block'} p-4 cursor-pointer rounded-lg 
        min-w-70 sm:min-w-85 shrink-0 shadow-lg transition duration-300 ease-in-out hover:transform hover:-translate-y-2`}>
            ...
            <p className={`${index % 2 === 0 ? 'text-beige' : 'text-bistre'} opacity-50 text-sm mb-2`}>{item.subject} • Kelas {item.grade} • Bab {item.bab}</p>
