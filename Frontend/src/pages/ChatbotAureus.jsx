...
const mdComponents = {
  ...
  ul: ({ children }) => <ul className="list-disc ml-4 text-sm">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal ml-4 text-sm">{children}</ol>,
  li: ({ children }) => <li className="mb-1">{children}</li>,
}

// Aureus AI avatar icon
const AureusIcon = () => (
  <div className="w-10 h-10 mr-2 flex items-center justify-center rounded-full bg-white shrink-0">
    <svg
...
export default function ChatbotUI() {
  ...
  const typeEffect = (text) => {
    ...
    setTypingText('')
    setIsTyping(true)
    setLoading(false) // loading spinner tidak perlu tampil saat sudah mengetik
  ...
  const getFromJSON = (input) => {
    ...
    Knowledge.forEach((item) => {
      let score = 0
      item.keywords.forEach((k) => { if (text.includes(k)) score++ })
  ...
  const sendMessage = async (text) => {
    ...
    // ─── Layer 1: JSON knowledge base ───────────────────────────────────────
    const jsonAnswer = getFromJSON(text)
    if (jsonAnswer) {
      setTimeout(() => typeEffect(jsonAnswer), 200)
      return
    }
    ...
    try {
      ...
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`,
        {
          ...
          body: JSON.stringify({
            system_instruction: {
              parts: [{ text: 'Kamu adalah AI tutor. Jawab singkat, jelas, maksimal 3 kalimat.' }],
            },
      ...
      // ✅ Dipanggil SEKALI — bug double-call sudah diperbaiki
          ...
          <div className="w-full max-w-2xl flex flex-col gap-5 mt-10 lg:mt-5 mb-20 ml-0 lg:ml-5 space-y-3 overflow-y-auto flex-1">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {msg.role === 'user' && (
                  <div className="w-8 h-8 rounded-full bg-icon ml-2 flex items-center justify-center overflow-hidden shrink-0">
                    <div className="w-9 h-9 lg:h-12 lg:w-12 md:h-12 md:w-12 mb-2 bg-bistre rounded-full overflow-hidden">
      ...
      <div className="container fixed flex flex-col mx-auto w-full md:left-20 lg:left-53 bottom-0 right-0 bg-white">
        ...
        >
          <input
            ...
            className="flex-1 border rounded-xl px-4 py-2 outline-none border-icon ring-1 ring-icon"
          />
          <button
            type="submit"
