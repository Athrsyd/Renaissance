...
import SkeletonLevelBanner from "../components/SkeletonLoading/ProgressPage/SkeletonXpSummaryCard";
import SkeletonXpSummaryCard from "../components/SkeletonLoading/ProgressPage/SkeletonXpSummaryCard";
import SkeletonStreakCard from "../components/SkeletonLoading/ProgressPage/SkeletonStreakCard";
import SkeletonAchievementsCard from "../components/SkeletonLoading/ProgressPage/SkeletonAchievementsCard";
import SkeletonClassProgressCard from "../components/SkeletonLoading/ProgressPage/SkeletonClassProgressCard";
import SkeletonTipsBar from "../components/SkeletonLoading/ProgressPage/SkeletonTipsBar";
...
const XpSummaryCard = ({ xp, weeklyData }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#9B7A5B]/15 p-5">
      ...
      <p className="text-[11px] text-bistre/50 mb-3">
      ...
      <p className="text-3xl font-bold font-monstserrat text-chamoisee underline decoration-chamoisee/30">
        {xp.toLocaleString()}{" "}
        <span className="text-base font-semibold">XP</span>
      </p>
...
const StreakCard = ({ currentStreak, longestStreak, weekly }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#9B7A5B]/15 p-5">
      ...
      <p className="text-[11px] text-bistre/50 mb-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <p className="text-3xl font-bold text-bistre font-monstserrat leading-tight">
            {currentStreak}{" "}
            <span className="text-sm font-normal text-bistre/50">Hari</span>
          </p>
          <p className="text-[11px] text-bistre/60">
        ...
        {WEEK_DAYS.map((day, i) => {
          ...
          return (
            <div key={day} className="flex flex-col items-center gap-1">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${isActive ? "bg-bistre text-white" : "bg-beige/60 text-bistre/20"}`}
              >
      ...
      <div className="bg-[#F8F3E0] rounded-xl px-3 py-2.5 flex items-center gap-2">
        ...
        <p className="text-[11px] text-bistre font-semibold">
          {daysLeft > 0
            ? `Belajar ${daysLeft} hari lagi untuk rekor baru`
            : "Kamu pecahkan rekor! 🎉"}
        </p>
      </div>
    </div>
...
const AchievementBadge = ({ label, Icon, unlocked }) => (
  <div className="flex flex-col items-center gap-1.5">
    <div
      className={`w-14 h-14 rounded-2xl flex items-center justify-center ${unlocked ? "bg-bistre text-white shadow-sm" : "bg-beige/40 text-bistre/20 border border-bistre/10"}`}
    >
      {unlocked ? <Icon size={22} /> : <Lock size={18} />}
    </div>
    <span
      className={`text-[10px] font-semibold text-center leading-tight ${unlocked ? "text-bistre" : "text-bistre/30"}`}
    >
      {label}
    </span>
  </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#9B7A5B]/15 p-5">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="font-bold text-bistre text-sm">
        ...
        {progressPersen >= 80 && (
          <div className="shrink-0 bg-green-50 border border-green-200 rounded-xl px-3 py-2 text-center ml-2">
            ...
            <p className="text-[9px] text-green-600/70 mt-0.5 leading-snug">
              Selesaikan semua mata
              <br />
              pelajaran untuk naik
              <br />
              ke Kelas {nextKelas}!
            </p>
          </div>
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative shrink-0 w-28 h-28">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-xl font-bold text-bistre">
              {progressPersen}%
            </span>
        ...
        <div className="flex-1">
          <p className="text-xs text-bistre font-semibold mb-1">
            {modulsSelesai} / {totalModuls} mata pelajaran selesai
          </p>
          <div className="flex gap-1 flex-wrap mb-2">
            {Array.from({ length: totalModuls }).map((_, i) => (
              <div
                key={i}
                className={`h-2.5 flex-1 min-w-3 rounded-sm ${i < modulsSelesai ? "bg-bistre" : "bg-beige/60"}`}
              />
            ))}
          </div>
          <p className="text-[11px] text-bistre/50 mb-3">
            {modulsLeft} mata pelajaran lagi menyelesaikan Kelas {kelas}
          </p>
          {nextKelas && (
            <div className="flex items-center gap-2 bg-beige/40 rounded-xl px-3 py-2">
              <Lock size={11} className="text-bistre/40 shrink-0" />
              <span className="text-[11px] text-bistre/60 font-medium">
                Kelas {nextKelas} Terkunci
              </span>
            </div>
          )}
        </div>
...
const Progress = () => {
  ...
  const fetchStreak = useCallback(async () => {
    try {
      const r = await API.get("/streak", { headers: headers() });
  ...
  const fetchTime = useCallback(async () => {
    try {
      const r = await API.get("/quiz-time", { headers: headers() });
      setTimeData(r.data.data);
    } catch {}
  }, []);

  const fetchClass = useCallback(async () => {
    try {
      const r = await API.get("/progress/kelas", { headers: headers() });
      setClassData(r.data.data);
    } catch {}
  }, []);

  // Satu useEffect: jalankan semua fetch, baru matikan isLoading setelah semuanya selesai
  // (baik sukses maupun gagal, karena masing-masing fetch sudah punya try/catch sendiri)
  useEffect(() => {
    Promise.all([
      fetchXp(),
      fetchProgress(),
  ...
  const modulsSelesai = dataProgress.filter(
    (i) => Number(i.progress) === 100,
  ).length;
  const totalModuls = 18;
  const modulMTK = dataProgress.filter(
    (i) =>
      (i.mapel || "").toLowerCase() === "matematika" &&
      Number(i.progress) === 100,
  ).length;

  const achStatus = {
    modulSelesai: modulsSelesai,
  ...
  const classProgressTotalModuls = Math.max(totalModuls, 15);
    ...
    <>
      ...
      <div className="flex flex-col lg:ml-64 md:ml-20 bg-[#FBF9F6] min-h-screen pb-24 lg:pb-10">
        <div className="px-5 sm:px-8 lg:px-10 pt-6 max-w-5xl w-full mx-auto">
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-3xl font-bold font-monstserrat text-bistre">
              Progress
            </h1>
            <p className="text-sm text-bistre/50 mt-1">
              Pantau perjalanan belajarmu dan raih pencapaian terbaik!
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            >
              ...
              <div className="flex-1 w-full">
                <p className="text-[11px] text-bistre/50 uppercase tracking-wider font-semibold mb-0.5">
                  Level Saat Ini
                </p>
                <p className="text-4xl font-bold text-bistre font-monstserrat">
                  Level {level}
                </p>
                <p className="text-sm text-chamoisee font-semibold mb-3">
                  {getLevelTitle(level)}
                </p>
                <div className="w-full h-2.5 bg-white/60 rounded-full overflow-hidden border border-chamoisee/20">
                  <div
                    className="h-full bg-bistre rounded-full transition-all duration-700"
                    style={{ width: `${xpPct}%` }}
                  />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-[11px] text-bistre/50">
                    {xp.toLocaleString()} XP
                  </span>
                  <span className="text-[11px] text-bistre/50">
                    {(xp + xpToNext).toLocaleString()} XP
                  </span>
                </div>
              </div>
              <div className="hidden sm:block shrink-0 text-right max-w-36">
                <p className="text-xs font-bold text-bistre mb-1">
                  Kamu Hebat!
                </p>
                <p className="text-xs text-bistre/60 leading-snug">
                  Terus belajar untuk naik level dan unlock lebih banyak
                  pencapaian!
                </p>
              </div>
            </div>
          ...
          {/* Row 1: XP Summary | Streak */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {/* <XpOverviewCard /> */}
            {isLoading ? (
              <SkeletonXpSummaryCard />
            ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-white rounded-2xl border border-[#9B7A5B]/15 p-5">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[11px] text-bistre/50">
                      Achievement yang baru kamu raih
                    </p>
                  </div>
            ...
            {/* Class Progress */}
            {isLoading ? (
              <SkeletonClassProgressCard
                totalModuls={classProgressTotalModuls}
              />
            ) : (
              <ClassProgressCard
                kelas={classData.kelas ?? 10}
                progressPersen={parseFloat(
                  ((modulsSelesai / classProgressTotalModuls) * 100).toFixed(2),
                )}
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-[#9B7A5B]/15 px-5 py-4 flex items-center gap-3">
              ...
              <span className="text-xs text-bistre/55 ml-1">
                {getTipHariIni()}
              </span>
