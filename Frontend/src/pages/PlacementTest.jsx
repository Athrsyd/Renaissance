...
const StepSidebar = ({ currentStep, selectedKelas }) => {
  ...
  return (
    <>
      {/* ── Mobile/tablet: bar horizontal ringkas (di bawah lg) ── */}
      <aside className="lg:hidden w-full fixed top-0 left-0 bg-bistre flex items-center justify-between px-4 py-3 gap-3 z-30">
        <a href="/" className="flex items-center gap-2 text-white font-semibold shrink-0">
          <img src={Logo} alt="logo" className="w-8 h-8 object-contain" />
          <h1 className="text-sm font-semibold font-jakarta">Renaissance</h1>
        </a>

        <div className="flex items-center gap-2">
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 border-2
              ${currentStep >= 1 ? 'bg-khaki border-khaki text-bistre' : 'border-khaki/40 text-khaki/40'}`}>
            {currentStep > 1 ? <CheckCircle size={12} /> : '1'}
          </div>
          <div className="w-4 h-px bg-khaki/30" />
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 border-2
              ${currentStep >= 2 ? 'bg-khaki border-khaki text-bistre' : 'border-khaki/40 text-khaki/40'}`}>
            {currentStep > 2 ? <CheckCircle size={12} /> : '2'}
          </div>
        </div>

        <button type="button" onClick={() => handleLogout()}
          className="flex items-center justify-center w-8 h-8 rounded-xl text-beige/80 hover:bg-khaki/60 hover:text-white transition duration-300 shrink-0">
          <LogOut size={18} strokeWidth={1.8} />
        </button>
      </aside>

      {/* ── Desktop: sidebar penuh, tidak berubah (lg ke atas) ── */}
      <aside className="hidden lg:flex w-60 shrink-0 fixed h-screen bg-bistre flex-col items-start pt-6 pb-8 px-5 gap-12 z-20">
        <div>
...
const PilihKelas = ({ onLanjut }) => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto w-full">
      <div className="px-4 sm:px-6 lg:px-10 pt-10 pb-6">
        <h1 className="text-xl lg:text-2xl font-bold text-bistre">Halo, Selamat Datang!</h1>
        <p className="text-bistre/60 text-sm mt-1">Sebelum mulai belajar, pilih kelasmu terlebih dahulu.</p>
        {error && <p className="text-red-500 text-sm mt-2">{error}</p>}
      </div>

      <div className="px-4 sm:px-6 lg:px-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 flex-1">
        {KELAS_LIST.map((item) => (
          <button key={item.kelas} onClick={() => setSelected(item.kelas)}
            className={`relative flex flex-col items-center justify-center lg:scale-95 h-auto lg:h-100 py-10 lg:py-6
              lg:hover:scale-96 hover:shadow-2xl text-left p-6 rounded-2xl border-2 transition-all duration-300
              ${selected === item.kelas
                ? 'border-bistre bg-bistre/1 shadow-2xl lg:scale-100'
                : 'border-beige bg-white hover:border-khaki'}`}
          >
            <div className="flex justify-center items-center text-center flex-col gap-1">
              <p className="text-2xl lg:text-3xl font-bold text-bistre">{item.label}</p>
              <p className="text-base lg:text-lg font-semibold text-chamoisee mb-3">{item.jenjang}</p>
              <p className="text-sm lg:text-md text-black leading-relaxed">{item.deskripsi}</p>
            </div>
            <div className={`relative lg:absolute lg:-translate-x-1/2 lg:-translate-y-1/2 lg:top-85/100 lg:left-1/2 mt-4 lg:mt-0 w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all shrink-0
      ...
      <div className="px-4 sm:px-6 lg:px-10 py-6 flex justify-center">
        <button onClick={handleLanjut} disabled={!selected || loading}
          className="bg-bistre text-beige font-semibold py-3 px-10 lg:px-16 rounded-xl hover:bg-bistre/90 disabled:opacity-40 transition-all flex items-center gap-2 w-full sm:w-auto justify-center">
...
const TesAwal = ({ kelas, onSelesai, onLewati, onKembali }) => {
  ...
  if (showMapelIntro && currentMapel) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-6 px-4 sm:px-10 w-full">
      <div className="bg-bistre/5 border-2 border-khaki/30 rounded-2xl px-6 sm:px-16 py-10 text-center max-w-md w-full">
        <p className="text-xs font-semibold text-chamoisee uppercase tracking-wider mb-2">
          Mapel {mapelIdx + 1} dari {totalMapel}
        </p>
        <h2 className="text-2xl sm:text-3xl font-bold text-bistre mb-1">{currentMapel.mapel}</h2>
        ...
        <button onClick={() => setShowMapelIntro(false)}
          className="bg-bistre text-beige font-semibold py-2.5 px-12 rounded-xl hover:bg-bistre/90 transition-all w-full sm:w-auto">
          Mulai
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#FAF5F0]">
      ...
      {/* ── TOP NAVBAR ── */}
      <div className="flex flex-row flex-wrap items-center justify-between gap-2 px-3 sm:px-6 py-3 mt-14 lg:mt-0 bg-[#FAF5F0] border-b border-beige/60">
        {/* Kembali — kembali ke mapel intro */}
        <button
          onClick={() => {
            onKembali();
          }}
          className="bg-bistre transition-all duration-300 lg:hover:-translate-x-1 flex flex-row items-center gap-1.5 sm:gap-2 text-white px-3 sm:px-5 py-2 rounded-full text-xs sm:text-sm font-medium"
        >
          <ArrowLeft size={16} />
        ...
        {/* <div className="relative flex items-center"> */}
          <button
            ...
            disabled={submitting}
            className="text-xs sm:text-sm flex flex-row justify-center items-center gap-1.5 sm:gap-2
             font-medium text-white transition lg:hover:translate-x-1 disabled:opacity-40 bg-chamoisee py-2 px-3 sm:px-4 rounded-full"
          >
            {submitting ? 'Menyimpan...' : 'Lewati Test'}
            <ArrowRight size={16} />
          </button>
        {/* </div> */}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button className="w-9 h-9 hidden sm:flex items-center justify-center rounded-full hover:bg-beige transition">
          ...
          <div className="flex items-center gap-1">
            ...
            <img src={RowBottom} alt="dropdown" className="w-4 h-4 opacity-60 hidden sm:block" />
          </div>
        </div>
      </div>

      {/* ── BODY ── */}
      {/* Mobile/tablet: kolom tunggal, scroll wajar. Desktop (lg): dua kolom, panel kanan fixed — identik versi awal. */}
      <div className="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden">

        {/* ── SOAL AREA ── */}
        <div className="flex-1 lg:overflow-y-auto px-4 sm:px-6 py-6 lg:mr-72">
          <div className="bg-white rounded-2xl border border-beige shadow-sm p-4 sm:p-6 max-w-2xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-5 gap-2">
              ...
              <span className="bg-bistre/10 text-bistre text-xs font-semibold px-3 py-1 rounded-full text-right">
                {currentMapel.mapel}
              </span>
            </div>
            ...
            <div className="bg-bistre/80 border-2 border-coffe rounded-2xl p-4 sm:p-5">
              {currentSoal ? (
                <RenderSoal
        ...
        {/* ── PANEL KANAN ── */}
        {/* Mobile/tablet: mengalir normal di bawah area soal. Desktop (lg): fixed di kanan, persis versi awal. */}
        <div className="w-full lg:fixed lg:right-0 lg:top-0 lg:pt-16 lg:w-72 lg:h-screen border-t lg:border-t-0 lg:border-l border-beige bg-white flex flex-col p-4 sm:p-5 gap-4 lg:overflow-y-auto lg:z-10">
              ...
              {currentMapel?.soal.map((_, i) => {
                ...
                return (
                  <button key={i}
                    ...
                    className={`w-full aspect-square lg:w-12 lg:h-12 rounded-full text-sm font-bold transition-all
          ...
          {/* Legend */}
          <div className="flex flex-row lg:flex-col flex-wrap gap-x-4 gap-y-1.5">
...
const Selesai = ({ kelas, result, onMasukDashboard }) => {
  ...
  return (
    <div className="flex-1 flex items-center justify-center px-4">
      <div className="text-center max-w-sm w-full flex flex-col items-center gap-5">
        ...
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-bistre">Tes Awal Selesai!</h2>
...
const PlacementTest = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#FAF5F0]">
      <StepSidebar currentStep={step} selectedKelas={selectedKelas} />

      <div className="container lg:ml-60 mt-14 lg:mt-0 lg:mr-6 w-full lg:w-4/5 mx-auto flex justify-center items-center">
