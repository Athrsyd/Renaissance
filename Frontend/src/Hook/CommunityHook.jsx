...
const CommunityHook = () => {
    ...
    const fetchCommunities = async () => {
        try {
            ...
            setError(error.response?.data?.message || 'Gagal memuat komunitas')
            setCommunities([])
        } finally {
            setIsLoading(false)
        }
    ...
    const searchCommunities = async (query = '') => {
        ...
        try {
            ...
            const response = await API.get(
                `/communities/search?name=${encodeURIComponent(query)}`,
                { headers: { Authorization: `Bearer ${tokenRenaissance}` } }
            )
    ...
    const fetchPopularCommunities = async () => {
        ...
        try {
            ...
            setError(error.response?.data?.message || 'Gagal mengambil komunitas populer')
            setSearchResults([])
        } finally {
            setIsLoading(false)
        }
